...
namespace atl
{
  ...
  void AtlPassthroughComponent::subJoystickCb(sensor_msgs::msg::Joy::SharedPtr &&msg)
  {
    ...
    // Print incoming axes so it's visible when the node starts (INFO level)
    ...
    // Determine the controller type based on the 4th axis
    bool isXbox = (msg->axes[5] == 1.0);
    // RCLCPP_INFO(get_logger(), "Controller detected: %s", isXbox ? "Xbox" : "Logitech");
    ...
    if (isXbox)
    {
      ...
      button2 = msg->buttons[0]; // Use button 0
      button1 = msg->buttons[3]; // Use button 2
    }
    else
    {
